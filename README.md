...
This repository contains a collection of research papers developed during my Computer Science Master's program at NYU, focusing on  topics in Machine Learning, Deep Learning, Natural Language Processing, Information Retrieval, and Intelligent Systems. Each project explores fundamental challenges in AI and demonstrates practical applications of advanced computational techniques to real world problems at scale.
...
### 4. Building a Disk-Based Search Engine for the MS MARCO Dataset
...
### 5. A Comparative Analysis of Sparse, Dense, and Hybrid Ranking Pipelines for MS MARCO
...
### 6. A Comprehensive Analysis of Dynamic Tiered BM25 Indexing and Query Routing using MSMARCO

**Paper:** [example_6.pdf](example_6.pdf)  
**Repository:** https://github.com/timothycao/search-systems

This research conducts a comprehensive study of a production-inspired pipeline for dynamic BM25 indexing with tiered partitions and delta shards that handle freshly ingested documents. The main objective is to evaluate the quality versus cost trade-offs of both learned dynamic document tiering and learned query routing in large-scale information retrieval systems. For learned dynamic document tiering, the study trains an XGBoost model to assign documents to quality tiers (T1 for high-quality documents, T2 for lower-quality documents), specifically designed to capture class imbalances accurately given that T1 should contain fewer documents than T2. For learned query routing, the research trains 10 different logistic regression classifiers derived as a function of self-labeled query training data using varying Recall@100 threshold variances, enabling the system to dynamically decide between searching only T1 or searching both T1 and T2 in parallel based on query characteristics. Delta shards periodically roll freshly ingested documents into base indexes when thresholds are exceeded. At query time, the system employs an overfetching strategy that retrieves more candidates than the final top-k requirement from both the tiered indexes (T1 and T2) and their respective delta indexes, then rescores all candidates with global statistics computed across the entire collection—this overfetching is necessary to compensate for the document sparsity introduced by splitting the corpus between T1 and T2 indexes and their respective delta indexes, ensuring that high-quality results are not missed due to the fragmented index structure. The learned query router then optimizes retrieval decisions based on the rescored results. The study compares tiered retrieval against a non-tiered BM25 baseline built on the same corpus, evaluating the multiple learned query routing models with varying classification routing thresholds. By separating the corpus into training and working subsets and ensuring all evaluation documents are included in the working set, the research aims to quantify how the combination of learned dynamic document tiering and intelligent query routing can balance retrieval quality with computational efficiency in large-scale information retrieval systems.

### 7. Code Sensei - Distributed AI Programming Assessment Handler
